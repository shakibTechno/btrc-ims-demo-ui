// ─── BLBTSSitesOverlay ───────────────────────────────────────────────
// 15,154 Banglalink BTS sites (BTRC submission, Dec 2025).
// Microwave (13,015) → amber   Fiber (2,113) → blue   Inactive (26) → grey
...
export type BLBTSTxType   = 'Microwave' | 'Fiber' | 'Inactive'
export type BLBTSTxFilter = Set<BLBTSTxType>

export const BL_BTS_COLORS: Record<BLBTSTxType, string> = {
  Microwave: '#f59e0b',
  Fiber:     '#2563eb',
  Inactive:  '#94a3b8',
}

const BL_BTS_LABELS: Record<BLBTSTxType, string> = {
  Microwave: 'Microwave',
  Fiber:     'Fiber',
  Inactive:  'Inactive',
}
...
interface Props {
  visible:  boolean
  txFilter: BLBTSTxFilter
}

export default function BLBTSSitesOverlay({ visible, txFilter }: Props) {
  const [data, setData] = useState<FeatureCollection | null>(null)

  useEffect(() => {
    if (!visible || data) return
    fetch('/data/bl-bts-sites.geojson?v=1')
      .then(r => r.json()).then(setData)
      .catch(err => console.warn('BLBTSSitesOverlay: fetch failed', err))
  }, [visible, data])
  ...
  const filtered = useMemo(() => {
    ...
    return {
      ...data,
      features: data.features.filter(f => txFilter.has(f.properties?.tx as BLBTSTxType)),
    }
  }, [data, txFilter])
  ...
  const pointToLayer = useCallback((_feat: Feature, latlng: L.LatLng): L.Layer => {
    const tx = _feat.properties?.tx as BLBTSTxType
    return L.circleMarker(latlng, {
      radius:      2.5,
      fillColor:   BL_BTS_COLORS[tx] ?? '#94a3b8',
      color:       'white',
      weight:      0.5,
      fillOpacity: 0.85,
      renderer,
  ...
  const onEach = useCallback((feat: Feature, layer: L.Layer) => {
    ...
    const tx       = p.tx as BLBTSTxType
    const label    = BL_BTS_LABELS[tx] ?? tx
